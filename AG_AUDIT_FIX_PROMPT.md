# AG Prompt — Account-Read Reconciliation Audit Fixes, Round 2

Read `MOTIM_ACCOUNT_READ_AUDIT.md` first, including **Round 2**. It is the authoritative feedback for commit `770f700`.

Pull `origin/main` first and stop if the tree is not clean. Fix all 2 MEDIUM and 1 LOW Round 2 findings with focused regression tests. Preserve the offline-only boundary: no capture, browser sign-in, network/sockets, replay, auth-value handling, or real-capture runbook. Update `motim-account-read-report.md` with exact fix/test evidence.
